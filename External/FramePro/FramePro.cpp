...
namespace FramePro
{
	...
	public:
		...
		void SetAffinity(int64 affinity);
...
namespace FramePro
{
	...
	public:
		...
		void SetThreadAffinity(int64 affinity);
	...
	private:
		...
		void Disconnect(bool wait_for_threads_to_exit=true, bool shutting_down=false);

		void Disconnect_NoLock(bool wait_for_threads_to_exit=true, bool shutting_down=false);
...
//------------------------------------------------------------------------
void FramePro::SetThreadAffinity(int64 affinity)
{
...
namespace FramePro
{
	...
	//------------------------------------------------------------------------
	void FrameProSession::Disconnect(bool wait_for_threads_to_exit, bool shutting_down)
	{
		CriticalSectionScope lock(m_CriticalSection);
		if(g_Connected)
			Disconnect_NoLock(wait_for_threads_to_exit, shutting_down);
	}

	//------------------------------------------------------------------------
	void FrameProSession::Disconnect_NoLock(bool wait_for_threads_to_exit, bool shutting_down)
	{
		if(mp_ContextSwitchRecorder)
	...
	//------------------------------------------------------------------------
	void FrameProSession::SetThreadAffinity(int64 affinity)
	{
...
namespace FramePro
{
	...
	//------------------------------------------------------------------------
	void Thread::SetAffinity(int64 affinity)
	{
...
namespace FramePro
{
		...
		ModuleCallbackContext* p_context)
	{
		...
		size_t module_name_length = FramePro_Min(strlen(p_module_name) + 1, sizeof(p_module_packet->m_ModuleName));
		memcpy(p_module_packet->m_ModuleName, p_module_name, module_name_length);
		p_module_packet->m_ModuleName[module_name_length - 1] = '\0';
	...
	int EnumerateLoadedModulesCallback(struct dl_phdr_info* info, size_t size, void* data)
	{
		...
		if (first)
		{
			...
			size_t result = readlink(arg1, char_filename, FRAMEPRO_MAX_PATH - 1);
			(void)result;
	...
	void EnumModulesLinux::EnumerateModules(Array<ModulePacket*>& module_packets, Allocator* p_allocator)
	{
		...
		if (!module_packets.GetCount())
		{
			...
			char arg1[20];
			sprintf(arg1, "/proc/%d/exe", getpid());
			memset(p_module_packet->m_ModuleName, 0, FRAMEPRO_MAX_INLINE_STRING_LENGTH);
			size_t result = readlink(arg1, p_module_packet->m_ModuleName, FRAMEPRO_MAX_INLINE_STRING_LENGTH - 1);
			(void)result;
			...
			char filename[FRAMEPRO_MAX_PATH + 8];
			sprintf(filename, "%s.sym_txt", p_last_slash);
			memcpy(p_module_packet->m_SymbolFilename, filename, strlen(filename) + 1);
			int copy_length = FramePro_Min(strlen(filename) + 1, sizeof(p_module_packet->m_SymbolFilename));
			memcpy(p_module_packet->m_SymbolFilename, filename, copy_length);
			p_module_packet->m_SymbolFilename[copy_length - 1] = '\0';
...
//------------------------------------------------------------------------
	...
	namespace FramePro
	{
		...
		namespace GenericPlatform
		{
			...
			//------------------------------------------------------------------------
			void SetThreadAffinity(void* p_os_thread_mem, int64 affinity)
			{
				HANDLE& handle = GetOSThread(p_os_thread_mem);
				...
				#else
					SetThreadAffinityMask(handle, (int)affinity);
				#endif
			}
	...
	namespace FramePro
	{
		...
		namespace GenericPlatform
		{
			...
			//------------------------------------------------------------------------
			void SetThreadAffinity(void*, int64)
			{
				// not implemented
			}
...
//------------------------------------------------------------------------
	...
	namespace FramePro
	{
		...
		//------------------------------------------------------------------------
		void Platform::SetThreadAffinity(void* p_os_thread_mem, int64 affinity)
		{
...
//------------------------------------------------------------------------
	...
	namespace FramePro
	{
		...
		{
			FILE* p_file = GetOSFile(p_os_file_mem);
			size_t result = fread(p_data, size, 1, p_file);
			(void)result;
		}
