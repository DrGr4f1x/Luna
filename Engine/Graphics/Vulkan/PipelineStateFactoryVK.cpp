...
#include "PipelineStateFactoryVK.h"
...
PipelineStateFactory::PipelineStateFactory(IResourceManager* owner, CVkDevice* device)
	: m_owner{ owner }
	, m_device{ device }
{
...
ResourceHandle PipelineStateFactory::CreateGraphicsPipeline(const GraphicsPipelineDesc& pipelineDesc)
{
	...
	VkGraphicsPipelineCreateInfo pipelineCreateInfo{
		...
		.layout					= GetVulkanResourceManager()->GetPipelineLayout(pipelineDesc.rootSignature.get()),
	...
	// Create handle and store cached data
	{
		...
		m_graphicsPipelineStates[index] = pipeline;

		return std::make_shared<ResourceHandleType>(index, IResourceManager::ManagedGraphicsPipeline, m_owner);
	}
}
...
VkPipeline PipelineStateFactory::GetGraphicsPipeline(uint32_t index) const
{
...
wil::com_ptr<CVkPipelineCache> PipelineStateFactory::CreatePipelineCache() const
{
