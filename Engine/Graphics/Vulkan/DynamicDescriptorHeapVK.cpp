...
void DefaultDynamicDescriptorHeap::ParseRootSignature(const RootSignature& rootSignature, bool graphicsPipe)
{
	...
	auto rootSignatureManager = GetVulkanRootSignatureManager();

	const uint32_t pipeIndex = graphicsPipe ? 0 : 1;
	const auto& rootSignatureDesc = rootSignatureManager->GetDesc(handle.get());

	// Get the pipeline layout
	m_pipelineLayout[pipeIndex] = rootSignatureManager->GetPipelineLayout(handle.get());
	...
	{
		// Get layout
		auto layout = rootSignatureManager->GetDescriptorSetLayout(handle.get(), rootParamIndex);
		...
		const auto& bindings = rootSignatureManager->GetLayoutBindings(handle.get(), rootParamIndex);
