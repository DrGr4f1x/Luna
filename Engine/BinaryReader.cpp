...
BinaryReader::BinaryReader(const std::byte* dataBlob, size_t dataSize)
{
	m_pos = dataBlob;
...
HRESULT BinaryReader::ReadEntireFile(const string& fileName, unique_ptr<std::byte[]>& data, size_t* dataSize)
{
	...
	data.reset(new std::byte[fileSize.LowPart]);
