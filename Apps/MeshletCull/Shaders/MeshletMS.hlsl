...
VertexOut GetVertexAttributes(uint meshletIndex, uint vertexIndex)
{
    ...
    float4 positionWS = mul(Instance.World, float4(v.Position, 1));

    VertexOut vout;
    vout.PositionVS = mul(Constants.View, positionWS).xyz;
    vout.PositionHS = mul(Constants.ViewProj, positionWS);
    vout.Normal = mul(Instance.WorldInvTrans, float4(v.Normal, 0)).xyz;
