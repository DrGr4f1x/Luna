...
)
{
    ...
    if (gid >= Options.MeshletCount)
        return;

    CullData c = MeshletCullData[Options.MeshletOffset + gid];
    ...
    if (gtid < vertCount)
    {
        float3 center = mul(World, float4(c.BoundingSphere.xyz, 1)).xyz; // Center point of cone
        ...
        axis = normalize(mul(World, float4(normalCone.xyz, 0))).xyz;
        ...
        DebugVertex v;
        v.position = mul(ViewProj, float4(pointWS, 1));
        v.color = Color;
